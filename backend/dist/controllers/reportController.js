...
const fullMonthName = (date) => date.toLocaleString('en-US', { month: 'long' });
...
export const getAccountsReport = async (req, res) => {
    try {
        ...
        const cashFlowData = Array.from(monthKeys).map((month) => {
            ...
            const outflow = (sum(expenses.filter((item) => monthName(item.date) === month).map((item) => item.amount)) +
                sum(purchaseOrders.filter((item) => monthName(item.date) === month).map((item) => item.totalAmount))) / 100000;
...
export const getTargetReport = async (req, res) => {
    try {
        ...
        const weeklyProgress = Array.from(productions.reduce((map, item) => {
            const week = `Week ${Math.ceil(item.date.getDate() / 7)}`;
            map.set(week, (map.get(week) || 0) + item.amount);
            return map;
        }, new Map()), ([week, achieved]) => ({ id: week, week, target: 500, achieved, daily: Math.round(achieved / 7) }));
